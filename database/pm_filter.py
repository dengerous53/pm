...
async def next_page(bot, query):
    ...
    btn.insert(0, 
        [
            InlineKeyboardButton('🌟 ᴄʜᴀɴɴᴇʟ', url='https://t.me/TamilDub_Linkzz'),
    ...
    if query.data.startswith("file"):
        ...
        try:
            if (AUTH_CHANNEL or REQ_CHANNEL) and not await is_subscribed(client, query):
                ...
                shortbtn = [
                    [
                        InlineKeyboardButton(text=f"⚡ᴄʟɪᴄᴋ ʜᴇʀᴇ ꜰᴏʀ ᴛᴇʟᴇɢʀᴀᴍ ꜰɪʟᴇ⚡", url = await get_shortlink(f"https://telegram.me/{temp.U_NAME}?start={ident}_{file_id}"))
                    ],
                    [
                        InlineKeyboardButton(text=f"🤔ʜᴏᴡ ᴛᴏ ɢᴇᴛ ᴛᴇʟᴇɢʀᴀᴍ ꜰɪʟᴇ🤔", url=f"https://telegram.me/how_to_download_tutorial/2")
                    ]
                ]
                short_markup = InlineKeyboardMarkup(shortbtn)
                shorten_link = await get_shortlink(f"https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}")
                await client.send_photo(chat_id=query.from_user.id, photo="https://te.legra.ph/file/d7a7ac6132262d66866ba.jpg", caption=f"<b>==> Title : {files.file_name}\n\n==> File_Size : {get_size(files.file_size)}\n\n#Note: Idu Telegram File dhaan Website Ula Poii edukanum ‼️ Epdi Edukuradhu nu therilaya? Killa How to download irukum Paruga</b>", reply_markup=short_markup)
                await query.answer('Check In Private Message, I have sent files in Private Message\n\nநீங்கள் கேட்ட படத்தை உங்களுக்கு தனிப்பட்ட முறையில் அனுப்பிவிட்டேன்\n\nസ്വകാര്യ സന്ദേശം പരിശോധിക്കുക, ഞാൻ സ്വകാര്യ സന്ദേശത്തിൽ ഫയലുകൾ അയച്ചു', show_alert=True)
                return
            else:
                shortbtn = [
                    [
                ...
                short_markup = InlineKeyboardMarkup(shortbtn)
                shorten_link = "https://t.me/{temp.U_NAME}?start={ident}_{file_id}"
                await client.send_photo(chat_id=query.from_user.id, photo="https://te.legra.ph/file/d7a7ac6132262d66866ba.jpg", caption=f"<b>==> Title : {files.file_name}\n\n==> File_Size : {get_size(files.file_size)}\n\n#Note: Idu Telegram File dhaan Website Ula Poii edukanum ‼️ Epdi Edukuradhu nu therilaya? Killa How to download irukum Paruga</b>", reply_markup=short_markup)
                await query.answer('Check In Private Message, I have sent files in Private Message\n\nநீங்கள் கேட்ட படத்தை உங்களுக்கு தனிப்பட்ட முறையில் அனுப்பிவிட்டேன்\n\nസ്വകാര്യ സന്ദേശം പരിശോധിക്കുക, ഞാൻ സ്വകാര്യ സന്ദേശത്തിൽ ഫയലുകൾ അയച്ചു', show_alert=True)
        ...
        buttons = [[
            ...
            InlineKeyboardButton('ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ', url='https://t.me/tamil_movie_reques'),
            InlineKeyboardButton('♚ ᴏᴡɴᴇʀ ♚', url='https://t.me/haashim_999')
            ],[
            InlineKeyboardButton('⌬ sᴜᴘᴘᴏʀᴛ ⌬', url='https://t.me/BOTxCLOUDS')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        cap = f"<b>{message.from_user.mention} ʏᴏᴜʀ ʀᴇǫᴜᴇsᴛᴇᴅ ᴍᴏᴠɪᴇ ɴᴀᴍᴇ <code>{search}</code>\n\n<i>This Message Will be Deleted within 5 Minutes</i>\n\nＧＲＯＵＰ ＮＡＭＥ: {message.chat.title}</b>"
